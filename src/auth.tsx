...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const acceptSession = useCallback(async (session: Session | null) => {
    ...
    let offlineReceipt: OfflineReceipt | null = null
    if (isDesktopRuntime) {
      offlineReceipt = await issueOfflineReceipt(session.access_token)
      const { saveDesktopSession } = await import('./desktopAuth')
      await saveDesktopSession(session, offlineReceipt)
    }
    setState({ status: 'authenticated', user: session.user, session, configured: supabaseConfigured, offlineUntil: offlineReceipt?.payload.expiresAt ?? null, error: null })
  }, [])
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      try {
        ...
        if (data.session) { if (active) await acceptSession(data.session); return }
        if (isDesktopRuntime) {
          ...
          if (stored?.session?.refresh_token) {
            ...
            if (refreshed.session) { if (active) await acceptSession(refreshed.session); return }
          }
    ...
    void boot()
    const subscription = supabase?.auth.onAuthStateChange((_event, session) => { if (active) void acceptSession(session) }).data.subscription
    return () => { active = false; subscription?.unsubscribe(); unlistenDesktop?.() }
  }, [acceptSession])
  const run = async (task: () => Promise<{ error: Error | null }>) => { setState(current => ({ ...current, status: 'loading', error: null })); const result = await task(); if (result.error) { const error = result.error; setState(current => ({ ...current, status: 'anonymous', error: error.message })); throw error } }
  const value = useMemo<AuthContextValue>(() => ({ ...state,
    signIn: async (email, password) => { const client = supabase; if (!client) throw new Error('尚未配置 Supabase'); await run(async () => { const { error } = await client.auth.signInWithPassword({ email, password }); return { error } }) },
    signUp: async (email, password) => { const client = supabase; if (!client) throw new Error('尚未配置 Supabase'); await run(async () => { const { error } = await client.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}${window.location.pathname}#/login` } }); return { error } }) },
