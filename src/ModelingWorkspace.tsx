...
export default function ModelingWorkspace({ config, onImport, onOpenUv }: { config: CustomModelConfig; onImport: () => void; onOpenUv: () => void }) {
  ...
  const persistCurrentModel = useCallback(async () => {
    ...
    try {
      ...
      if (next.model.type === 'custom' && next.model.assetId === updated.id) { next.model = { ...next.model, revision: updated.updatedAt, bounds: updated.bounds, triangleCount: updated.triangleCount, meshCount: updated.meshCount, materialCount: updated.materials.length }; studio.setSnapshot(next, false) }
      return true
  ...
  const autoUnwrapAndOpen = async () => {
    ...
    try {
      ...
      if (await persistCurrentModel()) { runtime.setWarning(null); onOpenUv() }
