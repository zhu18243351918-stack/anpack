...
import { DEFAULT_SCENE_OBJECT_TRANSFORM, getSceneObjectDescriptors } from './sceneObjects'
import { useStudio } from './store'
import { createModelAsset, modelAssetToConfig, parseModelGlb } from './modelAssets'
import { isDesktopRuntime } from './runtime'
import { useAuth } from './auth'
import type { ArtworkSurface, BoxFace, CameraConfig, ExportConfig, ModelAssetRecord, ModelType, PackagingModelConfig, PackagingTemplate, ProceduralModelType } from './types'
...
function ArtworkPanel({ onError }: { onError: (s: string) => void }) {
  ...
  const sample = (kind: 'citrus' | 'mono') => {
    ...
    patch('artwork', { url: canvas.toDataURL('image/png'), name: kind === 'citrus' ? '柑橘饮品示例' : '黑白生活方式示例' })
  }
  ...
  return <div className="scene-object-editor">
    ...
    <div className="scene-object-list" role="listbox" aria-label="场景对象">
      {objects.map(object => {
        ...
        return <button key={object.id} role="option" aria-selected={selected.id === object.id} className={selected.id === object.id ? 'selected' : ''} onClick={() => select(object.id)}><i className={`scene-object-dot ${object.kind}`} /><span><b>{object.label}</b><small>{object.kind === 'product' ? '包装模型' : object.kind === 'pedestal' ? '展示台' : '装饰模型'}</small></span><em className={visible ? '' : 'hidden'}>{visible ? '显示' : '隐藏'}</em></button>
      })}
    </div>
    <div className="scene-object-current"><span><b>{selected.label}</b><small>拖动数值实时更新，也可在画布中点击对象选择</small></span><button className="scene-object-reset" onClick={reset}><RotateCcw size={13} />重置</button></div>
...
function ExportDialog({ onClose, onToast }: { onClose: () => void; onToast: (v: string) => void }) {
  ...
  const [busy, setBusy] = useState(false); const [outputHandle, setOutputHandle] = useState<FileHandleLike | null>(null); const controller = useRef<AbortController | null>(null)
...
export default function App() {
  ...
  const [exportOpen, setExportOpen] = useState(false); const [importOpen, setImportOpen] = useState(false); const [modelingStartOpen, setModelingStartOpen] = useState(false); const [cadImportOpen, setCadImportOpen] = useState(false); const [cadCreateOpen, setCadCreateOpen] = useState(false); const [converting, setConverting] = useState(false); const [projectSaving, setProjectSaving] = useState(false); const [accountMenuOpen, setAccountMenuOpen] = useState(false); const [toast, setToast] = useState(''); const [error, setError] = useState('')
  ...
  const selectTemplate = (template: Exclude<PackagingTemplate, 'custom'>) => { patch('template', template); const type: ProceduralModelType = template === 'bottleLabel' ? 'bottle' : template === 'canLabel' ? 'can' : template === 'pouch' || template === 'shoppingBag' ? 'pouch' : 'box'; if (snapshot.model.type !== type) chooseModel(type); const nextFace = TEMPLATE_META[template].faces[0]; if (nextFace) setSelectedFace(nextFace) }
  const selectModel = (type: ProceduralModelType) => { chooseModel(type); patch('template', type === 'bottle' ? 'bottleLabel' : type === 'can' ? 'canLabel' : type === 'pouch' ? 'pouch' : 'carton') }
  const activateCustomAsset = (asset: ModelAssetRecord) => {
    const studio = useStudio.getState(); const next = structuredClone(studio.snapshot); next.model = modelAssetToConfig(asset); next.template = 'custom'; studio.setSnapshot(next); setImportOpen(false); setModelingStartOpen(false); setEditorMode('modeling'); setToast('模型已准备好，可开始编辑')
  }
