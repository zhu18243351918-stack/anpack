...
import { Box, Check, ChevronDown, CircleHelp, Download, FileUp, FolderOpen, ImagePlus, Lightbulb, PackageOpen, Play, Redo2, RotateCcw, Save, Search, Sparkles, Undo2, Upload, Wine, Wrench, X } from 'lucide-react'
...
export default function App() {
  ...
  const [exportOpen, setExportOpen] = useState(false); const [importOpen, setImportOpen] = useState(false); const [modelingStartOpen, setModelingStartOpen] = useState(false); const [cadImportOpen, setCadImportOpen] = useState(false); const [cadCreateOpen, setCadCreateOpen] = useState(false); const [converting, setConverting] = useState(false); const [projectSaving, setProjectSaving] = useState(false); const [toast, setToast] = useState(''); const [error, setError] = useState('')
